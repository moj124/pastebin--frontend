...
interface SelectOptions {
    options: ({value: null,label: string} | {value: number,label: string})[];
}

export default function Select({options}: SelectOptions): JSX.Element {
    const select_options = options.map((element,index)=>  <span key={index} className={styles.custom_option} data-value={element.value}>{element.label}</span>);
